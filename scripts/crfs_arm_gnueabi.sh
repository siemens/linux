...
usage_exit() {
	...
	echo "Incorrect parameters"
        echo "Usage: crfs_arm_gnueabi.sh <ROOTFS_PATH> <GNUEABI_PATH> <BUSYBOX_PATH>"
	echo
        echo "Examples: "
				echo
        echo "1. Little endian: "
				echo
        echo " ./crfs_arm_gnueabi.sh /tftpboot/rootfs_arm-gnueabi /swtools/devtools/gnueabi/arm_le/arm-linux-gnueabi /swtools/devsources/root_fs/files/busybox "
        echo ""
        echo "2. Big endian: "
				echo
        echo " ./crfs_arm_gnueabi.sh /tftpboot/rootfs_arm-gnueabi-be /swtools/devtools/gnueabi/arm_be/armbe-linux-gnueabi /swtools/devsources/root_fs/files/busybox "
        echo ""
        exit 1
}
...
then
  ...
  then
    ...
    echo " <GNUEABI_PATH> - path where GNUEABI 1.2.3 or newer package was installed."
    echo " (You need GNUEABI for dynamic libraries and cross-compiler.)."
    echo " GNUEABI tool kit for ARM can be downloaded from Marvell's extranet: https://www.marvell.com/login/index.jsp"
    echo ""
    echo "Example: "
    echo "./crfs_arm_gnueabi.sh /tftpboot/rootfs_arm-gnueabi /swtools/devtools/gnueabi/arm_le/arm-linux-gnueabi /swtools/devsources/root_fs/files/busybox "
    echo ""
...
dname=`dirname "$2"`
bname=`basename "$2"`
gnueabi="`cd \"$dname\" 2>/dev/null && pwd || echo \"$dname\"`/$bname"
if [ -e $gnueabi ] 
then
...
if [ `basename "${gnueabi}"` = armbe-linux-gnueabi ]; then
	eb=eb
	endian=be
...
gnueabi_lib=$gnueabi/arm${eb}-none-linux-gnueabi/sys-root/lib
gnueabi_lib2=$gnueabi/arm${eb}-none-linux-gnueabi/lib
gnueabi_prefix=$gnueabi/bin/arm${eb}-none-linux-gnueabi-

cd  $gnueabi_lib
cp -d libc-* libc.* libm*  ld-* libcrypt* libpthread* libdl* libSegFault* $lib_path

if [ $endian = "be" ]; then
	cp $HERE/lspci $HERE/show_endianess ${lib_path}/../usr/bin
	chmod +x ${lib_path}/../usr/bin/lspci
	chmod +x ${lib_path}/../usr/bin/show_endianess
else
	cp /swtools/devtools/gnueabi/arm_${endian}/rootfs/native/arm/sbin/lspci ${lib_path}/../sbin
fi
...
${gnueabi_prefix}strip ${lib_path}/l* > /dev/null 2>&1

cp -d $gnueabi_lib/libthread_db* $lib_path
...
touch ../${start_path}/../THIS_DIR_IS_FOR_GDBSERVER_SCRIPT

ln -s /lib ${d1}/lib

cat << EOF > ${usr_bin_path}/gdbserver
#!/bin/sh
export LD_LIBRARY_PATH=${start_path}/lib
gdbserver_bin \$*
...
toolchain=gnueabi ${endianess_string}
...
lib        = \${sdk_prefix}/arm${eb}-none-linux-gnueabi/libc/lib

cmd_prefix = \${sdk_prefix}/bin/arm${eb}-none-linux-gnueabi-
...
CROSS_COMPILER_PREFIX="$gnueabi_prefix"
...
#make TARGET_ARCH=arm CROSS=$gnueabi_prefix PREFIX=../. all install >/dev/null 2>/dev/null
make TARGET_ARCH=arm CROSS=$gnueabi_prefix PREFIX=../. all install
