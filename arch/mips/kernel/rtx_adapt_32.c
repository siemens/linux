...
#include <asm/mach-aud_soc/socGen.h>
...
void enable_soc_irq_cascade(unsigned int);
void mask_and_ack_soc_irq_cascade(unsigned int);
...
void show_regs_direct(struct pt_regs *regs);
void show_trace_direct(struct pt_regs *regs);
int printkDirect(char *String);
int printkDirectHex(unsigned info);
#endif
...
EXPORT_SYMBOL(per_cpu__rtx_percpu_data);
...
int __linux_timer_irq;	// the tick device may be different from the timer device
...
static void rtx_switch_htick_mode(enum clock_event_mode mode,
		struct clock_event_device *cdev);
#endif

DEFINE_PER_CPU(struct pt_regs, rtx_tick_regs);
int __rtx_timer_in_user_code;
...
// subroutines to store important information in a trace buffer
//#define CONFIG_RTX_USE_TRACE_BUFFER 1
#ifdef CONFIG_RTX_USE_TRACE_BUFFER

#define MAX_BUFFER_LEN 32
static int actIndex;
static struct myBuffStruct {
	unsigned info1, info2, info3, info4;
} traceBuffer[MAX_BUFFER_LEN];

int showTraceBuffer(void)
{
	int ii;
#if (1)
	printk("showTraceBuffer index=%d\n", actIndex);
	for (ii = 0; ii < MAX_BUFFER_LEN; ii++) {
		printk("info(%d): %#x:%#x:%#x:%#x\n", ii,
			traceBuffer[ii].info1, traceBuffer[ii].info2,
			traceBuffer[ii].info3, traceBuffer[ii].info4);
	}
#endif
#ifdef CONFIG_AUD_LOW_LEVEL_KERNEL_DUMP
	printkDirect("showTraceBuffer:");
	printkDirectHex(actIndex);
	printkDirect("\r\n");
	for (ii = 0; ii < MAX_BUFFER_LEN; ii++) {
		printkDirect(":");
		printkDirectHex(ii);
		printkDirect("::");
		printkDirectHex(traceBuffer[ii].info1);
		printkDirect(":");
		printkDirectHex(traceBuffer[ii].info2);
		printkDirect(":");
		printkDirectHex(traceBuffer[ii].info3);
		printkDirect(":");
		printkDirectHex(traceBuffer[ii].info4);
		printkDirect("\r\n");
	}  
#endif
...
void addToTraceBuffer(unsigned info1, unsigned info2,
	unsigned info3, unsigned info4)
{
	unsigned long flags;

	local_irq_save_hw(flags);

	traceBuffer[actIndex].info1 = info1;
	traceBuffer[actIndex].info2 = info2;
	traceBuffer[actIndex].info3 = info3;
	traceBuffer[actIndex].info4 = info4;
	actIndex++;
	if (actIndex >= MAX_BUFFER_LEN)
		actIndex = 0;
...
void rtx_init(void)
{
	...
	rtx_timer_init();
	// Setup of __rtx_timer_irq was handled before.
#ifdef CONFIG_RTX_DOMAIN_HRT
        __linux_timer_irq = IRQ_CPU_COMPARE;
#endif
...
restart_softirq:
	while (pcd->rtx_softirq_pending_words) {
		...
		while ((pending_part = pcd->rtx_softirq_pending[part])) {
			...
			//if (!--pcd->rtx_handler_depth[irq])
                          enable_soc_irq_cascade(irq);		// enable
			...
			if (irq == __rtx_timer_irq)
				// non hres: force linux timer interrupt
...
void resetCompareRegister(void);

// test whether this irq runs within rt domain
// can be used for lx timer interrupt handler to know
// whether the interrupt requests comes from rt
int inHardRealtimeMode(int irq)
{
	struct rtx_pcd *pcd = &__raw_get_cpu_var(rtx_percpu_data);
	return((unsigned) pcd->rtx_irqs[irq].handler);
}
...
int __rtx_handle_irq(int irq, struct pt_regs *regs)
{
	...
	mask_and_ack_soc_irq_cascade(irq);

	if (likely(pcd->rtx_irqs[irq].handler)) {

		trace_irq_entry_rt(-irq, regs, &rtDummyAction);
		//pcd->rtx_handler_depth[irq]++;
#ifdef CONFIG_SELECT_MIPS_COMPARE_TIMEBASE
		if (irq == IRQ_CPU_COMPARE) {
			// we need to reset the compare register to get a new compare interrupt
			// this may not be done during realtime timing handling
			resetCompareRegister();
		}
#endif
		...
		if (unlikely(rtx_get_cpu_var(rtx_mode) == LX_DOMAIN_SOFT)) {
			...
			if (irq != __rtx_timer_irq) {
				rtx_kstat_incr_irqs_this_cpu(irq, irq_desc + irq);
                            ...
                            log_interrupt_as_pending(irq);  // non hres: forces lx irq
			}
#endif
	...
	if (irq == __rtx_timer_irq) {
		...
		__rtx_timer_in_user_code = user_mode(regs);
		tick_regs->cp0_status = regs->cp0_status;
		if (pcd->rtx_curr_dom == RT_DOMAIN) {
			// disable tick interrupt state
			// why??
			tick_regs->cp0_status &= ~MIPS_IRQ_ENABLE_WORD;

			if (__rtx_timer_in_user_code)
				current->utime++;
			else
				current->stime++;
		}
				   ...
				   irqreturn_t (*nonrt_handler)(int, void *))
{
	...
	orgIrqFlags = irqflags;
	irqflags = irqflags & ~IRQF_RTIRQ_BALANCED;
	if (!(orgIrqFlags & IRQF_RTIRQ_BALANCED))
		irqflags |= IRQF_NOBALANCING;

	desc = irq_to_desc(irq);
	if (!desc->chip->mask_ack || !desc->chip->unmask) {
	...
	rtx_req_irqs[irq].flag = orgIrqFlags;
...
int rt_enable_irq_affinity(int irq)
{
	...
	if (!IS_REALTIME_PROCESS(current)
			&& !(rtx_req_irqs[irq].flag & IRQF_RTIRQ_BALANCED)) {
...
#ifdef CONFIG_SMP
	if (rtx_req_irqs[irq].flag & IRQF_RTIRQ_BALANCED) {
		// TBD set the rt handler to all cpus
	}
#endif
	...
	if (IS_RTX_TIMER_IRQ(irq)) {
		...
		if ((rtx_timer_freq < 224000000) || (rtx_timer_freq > 226000000)) {
			printkGen(NULL, "WARNING: tickdev frequency (%d) not yet supported\n",
				rtx_timer_freq);
		}
#endif
	}
...
#ifdef CONFIG_SMP
	if (rtx_req_irqs[irq].flag & IRQF_RTIRQ_BALANCED) {
		// TBD reset the rt handler to all cpus
	}
#endif
 ...
 * ( see arch/xxx/include/asm/irqflags.h).
...
void __rtx_local_irq_enable_no_handler(void)
{
	struct rtx_pcd *pcd;
        unsigned long flags;
...
void __rtx_local_irq_disable(void)
{
...
/*
 * All faults are handled in the Linux domain
 * e.g. an installed handler gets executed in the Linux domain.
 * The base assumption is, that within time critical paths a thread has to avoid
 ...
 */
int __rtx_trap_handling(int vector, struct pt_regs *regs, unsigned mode)
{
	unsigned int cpid;
	struct rtx_pcd *pcd;
	...
	if (IS_REALTIME) {
		switch(vector) {
		case RTX_TRAP_ALIGNMENT:
      			if (RT_UNALIGNMENT_VERBOSE)
				printkGen(NULL, "Unalignment exception (trap %d) in RT domain at epc=%#lx badvaddr=%#lx cause=%#lx status=%#lx\n",
					vector, regs->cp0_epc, regs->cp0_badvaddr,
					regs->cp0_cause, regs->cp0_status);
			if ((!mode) /*&& user_mode(regs)
				&& test_thread_flag(TIF_FIXADE)*/) {
				// here we may implement a fast rt alignment routine
			}
		...
		case RTX_TRAP_CPU:
			...
			// Handle floating point exception in RT.
			if (cpid == 1 && cpu_has_fpu) {
				// Assumption is that we have a FP coprocessor.
		      		if (RT_FP_EXCEPTION_VERBOSE)
		      			printkGen(NULL, "FP exception (trap %d) in RT domain at epc=%#lx cause=%#lx status=%#lx\n", vector, regs->cp0_epc, regs->cp0_cause, regs->cp0_status);

		      		local_irq_disable_hw();

				if (current->fpu_counter < 2)
					// m.n. currently no autoload during task switch
					// current->fpu_counter++;
		...
		case RTX_TRAP_RI: // illegal instruction
		case RTX_TRAP_MDMX:
		case RTX_TRAP_MT:
		case RTX_TRAP_DSP:

		case RTX_TRAP_ACCESS:
		default:
			if (RT_EXCEPTION_VERBOSE) {
				printkGen(KERN_ALERT,
				"fault %d in RT domain at pc=%#lx (xxx=%#x)\n",
				vector, regs->cp0_epc, regs->cp0_badvaddr);
			}
			current->rt_state3 |= RT_TASK_FAULT_DETECTED;
			rtx_migrate_to_lx(RTLX_TASK, NULL);
		...
		case RTX_TRAP_BREAK:
			if (RT_DEBUG_VERBOSE)
				printkGen(NULL, "Debug exception (trap %d) in RT domain at epc=%#lx badvaddr=%#lx cause=%#lx status=%#lx\n",
					vector, regs->cp0_epc, regs->cp0_badvaddr,
					regs->cp0_cause, regs->cp0_status);
#ifdef CONFIG_RTX_EXTENDED_RT_DEBUGGING_SUPPORT
...
/*
 * Restore the FP-state on context switch for a RT-thread
 * that applies FP arithmetics.
 *
 * currently a dummy: we wait for an exception within the new thread
 */
void rtx_fpu_state_restore(void)
{
#if (0)
	__enable_fpu();
	KSTK_STATUS(current) |= ST0_CU1;
...
//values set as defines in socGen.h 
/*--------------------------------------
** values for division
**    HRT_MULT  
**    HRT_SHIFT  
...
static int rtx_next_htick_shot(unsigned long delay, struct clock_event_device *cdev)
{
	//printkGen(NULL, "rtx_next_htick_shot del=%#x\n", delay);
...
static int verboseDecr = 1;
static unsigned long long myMaxNsValue = ((((unsigned long long) 0xffffffff)
                                     * HRT_MULT_REV) >> HRT_SHIFT_REV);
static unsigned long long myOverflowNsValue;
static unsigned myOldCntValue;
static unsigned minimalOffsetValue = 256;
...
static int rt_decr_register(void)
{
	...
	hres_aud_timer.irq = __rtx_timer_irq = __linux_timer_irq;
	myOverflowNsValue = 0;
	myOldCntValue = 0;
	if (mips_hpt_frequency) { // is in kHz
		hres_aud_timer.res_nsec = NSEC_PER_SEC / mips_hpt_frequency / 1000;
		if ((hres_aud_timer.res_nsec * mips_hpt_frequency * 1000) != NSEC_PER_SEC)
			hres_aud_timer.res_nsec++;
		if (verboseDecr)
			printkGen(NULL, "%s: resolution=%d\n", __func__,
				hres_aud_timer.res_nsec);
	}
	else {
		printkGen(NULL, "%s: WARNING: mips_freq not yet available\n", __func__);
		hres_aud_timer.res_nsec = 1;
	}
	return 0;
}
...
{
	if (verboseDecr)
		printkGen(NULL, "rtx_timer_start\n");
...
{
	u64 nsValue;
	unsigned actCntValue;
	unsigned long flags;

	local_irq_save_hw(flags);

	actCntValue = read_c0_count();
	if (actCntValue < myOldCntValue) {
		// we need a full 64 bit timer count
		myOverflowNsValue += myMaxNsValue;
	}
	myOldCntValue = actCntValue;
	nsValue = ((u64) actCntValue);
	//cntValue *= 5; // simple approach
	nsValue = nsValue * HRT_MULT_REV;
	nsValue >>= HRT_SHIFT_REV;
...
{
	unsigned long long calcValue;
	unsigned setValue;
	unsigned changeValue = 0;
	// signal correct timer setup
	int retVal = 1;
	unsigned long flags;

	if (newValue < minimalOffsetValue)
		newValue = minimalOffsetValue;

	local_irq_save_hw(flags);
restart:
	calcValue = (unsigned long long) newValue;
	calcValue = calcValue * HRT_MULT;
	calcValue >>= HRT_SHIFT;
	local_irq_save_hw(flags);
	setValue = (unsigned) calcValue + read_c0_count();
	write_c0_compare(setValue);

	if ((int)(read_c0_count() - setValue) > 0) {
		// signal early timer expiration
		retVal = 0;
		// be sure to have a valid new compare entry
		changeValue++;
		newValue <<= changeValue;
		//printkGen(NULL, "setRefValue too small (%#x:%d)\n",
			//newValue, changeValue);
		goto restart; 
	}
	local_irq_restore_hw(flags);
	...
	if (!retVal)
		printkGen(NULL, "%s: update new=%#x:%#x::%#x:%#x::%d\n", __func__,
			newValue, setValue, read_c0_compare(), read_c0_count(), changeValue);
	return(retVal);
}

void showHighResCounts(void)
{
	printkGen(NULL, "%s_a: compar=%#x:cnt=%#x\n", __func__,
		read_c0_compare(), read_c0_count());
#if (0)
	printkGen(NULL, "%s_b: compar=%#x:cnt=%#x\n", __func__,
		read_c0_compare(), read_c0_count());
#endif
}
