...
struct clock_event_device clockevent_mxc;
#else
...
#ifdef CONFIG_RTX_DOMAIN_HRT
struct clocksource clocksource_mxc = {
#else
...
#ifdef CONFIG_RTX_DOMAIN_HRT
unsigned long read_mxc_clksrc(void)
{
	return ((unsigned long)(mx3_get_cycles(NULL)));
}
#endif
...
printk("bh3672: mxc_clocksource_init rate = %d \n",c);
	if (timer_is_v2())
...
#ifdef CONFIG_RTX_DOMAIN_HRT
int mx3_set_next_event(unsigned long evt,
			      struct clock_event_device *unused)
#else
...
#ifdef CONFIG_RTX_DOMAIN
	if (likely(pcd->rtx_irqs[irq].handler)) ; // aknowledge in __rtx_handle_irq
	else gpt_irq_acknowledge();
#else
	gpt_irq_acknowledge();
#endif
...
static struct clock_event_device clockevent_mxc = {
	...
	.set_next_event	= mx3_set_next_event,
	.rating		= 200,
};
