...
extern int mv_prestera_initialize(void);
...
struct net_device_ops mvNetDevOps;
...
static int eth_get_bootargs_mac(MV_U8* mac_addr)
{
   char tmp_cmdline[COMMAND_LINE_SIZE];
   strlcpy(tmp_cmdline, saved_command_line, COMMAND_LINE_SIZE);

   if(strstr(tmp_cmdline,"mac=")==NULL){
      printk( KERN_NOTICE "    %s: cmdline without mac-address \n", __FUNCTION__);
      return 0;
   } else {
      ...
      while((single_arg = strsep(&zeile," "))!=NULL) {
         if(strstr(single_arg,"mac=")!=NULL) {
            ...
            *(mac_addr+0) = simple_strtol(strsep(&single_arg,"=: \0"),NULL,16);
            *(mac_addr+1) = simple_strtol(strsep(&single_arg,"=: \0"),NULL,16);
            *(mac_addr+2) = simple_strtol(strsep(&single_arg,"=: \0"),NULL,16);
            *(mac_addr+3) = simple_strtol(strsep(&single_arg,"=: \0"),NULL,16);
            *(mac_addr+4) = simple_strtol(strsep(&single_arg,"=: \0"),NULL,16);
            *(mac_addr+5) = simple_strtol(strsep(&single_arg,"=: \0"),NULL,16);

            printk( KERN_NOTICE "    %s: mac-address %02X:%02X:%02X:%02X:%02X:%02X \n",
                   __FUNCTION__, *(mac_addr+0),*(mac_addr+1),*(mac_addr+2),
...
static int eth_get_config(mv_eth_priv* priv, MV_U8* mac_addr)
{
    ...
    switch(priv->port)
    {
		...
		if(eth_get_bootargs_mac(mac_addr)) return mtu;
            	else mac_str = CONFIG_MV_ETH_0_MACADDR; 
	    else
...
static INLINE int eth_rx(struct net_device *dev, unsigned int work_to_do, int queue)
{
    ...
    while( work_done < work_to_do ) {
	    ...
	    mvNetDevOps.ndo_start_xmit(skb, out_dev);
        }
        else
        {
            skb->dev = dev;
...
irqreturn_t     mv_eth_interrupt_handler(int irq , void *dev_id)
{
    ...
    mv_net_priv         *net_priv = (mv_net_priv *)netdev_priv(dev);
...
/*XCAT board uses own init procedure*/
if( (boardId == DB_X500PROTOTYPE_ID) || (boardId == DB_98DX4122_ID) || (boardId == RD_98DX3121_ID) )
{
...
//    dev->weight = (CONFIG_MV_ETH_NUM_OF_RX_DESCR / 2);
    ...
    dev->netdev_ops = &mvNetDevOps;
    mvNetDevOps.ndo_open = mv_eth_open;
    mvNetDevOps.ndo_stop = mv_eth_stop;
    mvNetDevOps.ndo_start_xmit = eth_tx;
    mvNetDevOps.ndo_tx_timeout = eth_tx_timeout;
//    dev->poll = eth_poll;

    mvNetDevOps.ndo_set_mac_address = mv_eth_set_mac_addr;
    mvNetDevOps.ndo_set_multicast_list = mv_eth_set_multicast_list;
    mvNetDevOps.ndo_change_mtu = &mv_eth_change_mtu; 

    netif_napi_add(dev, &(net_priv->napi), eth_poll, CONFIG_MV_ETH_NUM_OF_RX_DESCR / 2);

#ifdef CONFIG_MV_GATEWAY
    if(priv->isGtw)
    {
        /* For Gateway driver replace some of callback functions */
	mvNetDevOps.ndo_open = mv_gtw_start;
        mvNetDevOps.nndo_stop = mv_gtw_stop;
        mvNetDevOps.ndo_set_mac_address = mv_gtw_set_mac_addr;
	mvNetDevOps.ndo_set_multicast_list = mv_gtw_set_multicast_list;
        mvNetDevOps.ndo_change_mtu = &mv_gtw_change_mtu;
