...
static struct mtd_info *sflash_probe(struct map_info *map)
{
	/* struct mtd_info *mtd = NULL; */   /* Moved outside of function to support LSP tests. a.y. - 12/05/09 */
	...
	mtd->flags = (MTD_WRITEABLE | MTD_BIT_WRITEABLE | MTD_POWERUP_LOCK); /* just like MTD_CAP_NORFLASH */
#else
...
//	lsec = (fsec +(instr->len / mtd->erasesize));
	...
	fsec = (MV_U32)div_u64_rem(instr->addr, mtd->erasesize, &rem);
	lsec = fsec + (MV_U32)div_u64_rem(instr->len, mtd->erasesize, &rem);
