...
static void default_idle(void)
{
	...
	if (!need_resched())
		__rtx_reset_stalled_bit();
	local_irq_disable_hw();
#endif

	if (!need_resched()) {
#ifdef CONFIG_RTX_DOMAIN
		{ // pending interrupts wait until the next interrupt occur
			struct rtx_pcd *pcd = &__raw_get_cpu_var(rtx_percpu_data);
			if(pcd->rtx_irq_pending_words) printk("Idle despite pending interrupts %x \n",pcd->rtx_irq_pending_words);
		}
#endif
		arch_idle();
	}
