...
/* uniform aud early printk CONFIG_AuD_LOW_LEVEL_PRINTK */
void do_putchar(char c){
	mv_putchar(c);
}

void do_putstr(const char *s){
	putstr(s);
}
...
extern MV_STATUS __init mvSwitchDecodingInit(void);
#endif
...
/*used to limit malloc strategy to overcommit algorithm*/
...
void fix_xcat_version(void)
{
	//
	// supporting older uboot where 0x7 is the boardid of DB_98DX4122
	// (and VERY unlikley that it will be RD_88F6281A_PCAC_ID). gc
	//
	extern int xcat_version;
	
	printk("xCat revision = A%d\n", (xcat_version > 0)); // no action - just print
	if (gBoardId == 0x7)
	{
		gBoardId = DB_98DX4122_ID;
		printk("BoardID fixed to %d\n\n", gBoardId);
	}
...
{
#if 0 /* use defaults - OVERCOMMIT_GUESS */
        /*Limit malloc strategy to never overcommit algorithm*/
        sysctl_overcommit_memory = OVERCOMMIT_NEVER;
        sysctl_overcommit_ratio = 95;
#endif
