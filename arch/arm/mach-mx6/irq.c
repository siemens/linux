...
// 2.6
static int mx6_gic_irq_set_wake(unsigned irq, unsigned int enable)
{
	if ((irq < MXC_INT_START) || (irq > MXC_INT_END)) {
		printk(KERN_ERR "Invalid irq number!\n");
		return -EINVAL;
	}

	if (enable) {
		gpc_wake_irq[irq / 32 - 1] |= 1 << (irq % 32);
		printk(KERN_INFO "add wake up source irq %d\n", irq);
	} else {
		printk(KERN_INFO "remove wake up source irq %d\n", irq);
		gpc_wake_irq[irq / 32 - 1] &= ~(1 << (irq % 32));
...
void mx6_init_irq(void)
{
	...
	for (i = MXC_INT_START; i <= MXC_INT_END; i++) {
		...
		// 2.6
		desc->chip->set_wake = mx6_gic_irq_set_wake;
	}
	mx6q_register_gpios();
}
