...
static void uic_unmask_irq(unsigned int virq)
{
	...
	sr = 1 << (31-src);
	rtx_spin_lock_irqsave(&uic->lock, flags);
	...
	rtx_spin_unlock_irqrestore(&uic->lock, flags);
}
...
void uic_irq_cascade(unsigned int virq, struct irq_desc *desc)
{
	...
	subvirq = irq_linear_revmap(uic->irqhost, src);
		...
		struct irq_desc *subdesc;
                subdesc = irq_to_desc(subvirq);
	        /* mask the interrupt. */
		if (subdesc->status & IRQ_LEVEL)
			subdesc->chip->mask(subvirq);
		else
			subdesc->chip->mask_ack(subvirq);

		// Interrupt logging (LX-interrupts)
		log_interrupt_as_pending(subvirq);
        }
#else
