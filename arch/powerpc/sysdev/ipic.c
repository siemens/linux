...
static RTX_DEFINE_SPINLOCK(ipic_lock);
#else
...
#ifdef CONFIG_RTX_DOMAIN
	rtx_spin_lock_irqsave(&ipic_lock, flags);
#else
...
#ifdef CONFIG_RTX_DOMAIN
	rtx_spin_unlock_irqrestore(&ipic_lock, flags);
#else
...
{
	struct ipic *ipic = ipic_from_irq(20);
	ipic_write(ipic->regs, IPIC_SEFCR, irqInfo);
}
