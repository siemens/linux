...
#ifdef CONFIG_RTX_DOMAIN
        // already done within the generic interrupt routine of RTX
#else
...
#ifdef CONFIG_RTX_DOMAIN
		printk("timer_interrupt-lost_interrupts callback not yet implemented\n");
#else
		do_IRQ(regs);
#endif
#endif
...
#if defined (CONFIG_RTX_DOMAIN) && defined (CONFIG_RTX_DOMAIN_HRT)
			// depending on active realtime domain we may
			// propagate the next timeout to the RT domain
			// or do it here
			if (rtx_repeat_next_htick_lx_shot(now))
				set_dec((int)now);
#else
			set_dec((int)now);
#endif
...
// we need to adjust the element .next_tb for use in the
// linux interrupt routine
int rtx_ppc_decrementer_set_next_event(unsigned long evt,
				      struct clock_event_device *dev)
{
