...
static RTX_DEFINE_SPINLOCK(ioapic_lock);		/* RTX_DOMAIN */
static RTX_DEFINE_SPINLOCK(vector_lock);		/* RTX_DOMAIN */

#if defined(CONFIG_RTX_DOMAIN) && defined(CONFIG_X86_32)
...
{
	return irq >= NR_IRQS && irq < RTX_VIRQ_BASE ?
			rtx_apic_irq_vector(irq) : irq_cfg(irq)->vector;
}
#endif
...
static void move_apic_irq(unsigned int irq)
{
	...
	if (desc->handle_irq == &handle_edge_irq) {
		rtx_spin_lock(&desc->lock);
		irq_complete_move(&desc);
		move_native_irq(irq);
		rtx_spin_unlock(&desc->lock);
		...
		rtx_spin_unlock(&desc->lock);
	} else if (desc->handle_irq != &handle_simple_irq)
		WARN_ON_ONCE(1);
}
#endif /* CONFIG_RTX_DOMAIN && CONFIG_SMP */
