...
#define ETH_LED_FLASH_TIME	2

/* function pointer for LED control */
int (*led_set_leds) (unsigned int, unsigned long*, unsigned long,
					unsigned long*, unsigned char, unsigned char,
					unsigned int, unsigned char) = NULL;
EXPORT_SYMBOL(led_set_leds);

extern void scw_set_startup_led_state(unsigned long led, int state);

extern int scw_get_hwtype(void);
#endif
...
#if defined(CONFIG_IBM_EMAC_MAL_QOS_V404)
	if (dev->rx_vchans) { //set RCBS sizes for all virtual channels as well
		int v;
		for (v = 1; v < dev->rx_vchans; v++) {
			mal_set_rcbs(dev->mal, v, emac_rx_size(new_mtu));
		}
	}
#endif
...
static void emac_clean_tx_ring(struct emac_instance *dev)
{
	...
	for (i = 0; i < NUM_TX_BUFF; ++i) {
		...
		dev->tx_desc[i].ctrl |= (i == (NUM_TX_BUFF - 1) ? MAL_TX_CTRL_WRAP : 0);
...
#ifdef CONFIG_SIMATIC_NET_SCALANCE_W
extern int global_sfp1_link;
extern int global_sfp2_link;
#endif

static void emac_print_link_status(struct emac_instance *dev)
{
#ifdef CONFIG_SIMATIC_NET_SCALANCE_W
	unsigned long leds[4] = {0,0,0,0};
	unsigned long led_state;
#endif

	if (netif_carrier_ok(dev->ndev)) {
#ifdef CONFIG_SIMATIC_NET_SCALANCE_W
		leds[0] |= LED_ETH_GREEN;
		led_state = LED_ON;
#endif
...
#ifdef CONFIG_SIMATIC_NET_SCALANCE_W
	/* If it's sfp based set two LEDs */
	/* DD: EAP-SFP has hw_type 8. The board types should be defined someplace
	 * and we should probably have a struct with common data e.g. hwtype that
	 * is filled up once at startup.
	 */
	if (scw_get_hwtype() == 8) {
		leds[0] = 0;
		if(global_sfp1_link)
		{
			leds[0] |= LED_ETH_SFP1_GREEN;
			led_state = LED_ON;
		}
		else
		{
			leds[0] |= LED_ETH_SFP1_GREEN;
			leds[0] |= LED_ETH_SFP1_YELLOW;
			led_state = LED_OFF;
		}
		if (led_set_leds) led_set_leds(SET_LED_STATE,leds,led_state,0,0,0,0,0);
		else    		  scw_set_startup_led_state(leds[0], led_state);
		leds[0] = 0;
		if(global_sfp2_link)
		{
			leds[0] |= LED_ETH_SFP2_GREEN;
			led_state = LED_ON;
		}
		else
		{
			leds[0] |= LED_ETH_SFP2_GREEN;
			leds[0] |= LED_ETH_SFP2_YELLOW;
			led_state = LED_OFF;
		}
		if (led_set_leds) led_set_leds(SET_LED_STATE,leds,led_state,0,0,0,0,0);
		else    		  scw_set_startup_led_state(leds[0], led_state);
...
static int emac_open(struct net_device *ndev)
{
	...
	if (dev->rx_vchans) { //set RCBS sizes for all virtual channels as well
		int v;
		for (v = 1; v < dev->rx_vchans; v++) {
			mal_set_rcbs(dev->mal, v, emac_rx_size(ndev->mtu));
		}
	}
#endif
...
#ifdef CONFIG_SIMATIC_NET_SCALANCE_W
				if (led_set_leds) {
					/* The LEDs we use to flash when receiving/transmitting packets */
					unsigned long leds_on[4] = {0,0,0,0};
					...
					if (scw_get_hwtype() == 8) {
						if(global_sfp1_link)
							leds_on[0] |= LED_ETH_SFP1_YELLOW;
						if(global_sfp2_link)
							leds_on[0] |= LED_ETH_SFP2_YELLOW;
					}
					else
					{
						leds_on[0] |= LED_ETH_YELLOW;
					}
					led_set_leds(SET_LED_FLASH,leds_on,0,NULL,0,0,0,ETH_LED_FLASH_TIME);
				}
#endif
		}
	}
...
static inline int emac_rx_sg_append(struct emac_instance *dev, int slot)
{
	if (likely(dev->rx_sg_skb != NULL)) {
        int maxLen = dev->rx_sg_skb->end - dev->rx_sg_skb->data;
		int len = dev->rx_desc[slot].data_len;
		int tot_len = dev->rx_sg_skb->len + len;

		if (unlikely(tot_len + 2 > maxLen /*dev->rx_skb_size*/)) {
 ...
 again:
...
#ifdef CONFIG_SIMATIC_NET_SCALANCE_W
		/* DD: Workaround for problem with junk packets after a link down and link up */
		if (unlikely(dev->safe)) {
			emac_recycle_rx_skb(dev,slot,len);
			dev->safe--;
			goto next;
		}
	...
	push_packet:
		...
		if (skb->pkt_type == PACKET_BROADCAST)
			++dev->scw_stats.rx_bcast_pkts;
		if (skb->pkt_type == PACKET_MULTICAST)
			++dev->scw_stats.rx_multi_pkts;
#endif
...
#ifdef CONFIG_SIMATIC_NET_SCALANCE_W
		if (len == 64)
			++dev->scw_stats.pkts_64;
		if ((len >= 65) && (len <= 127))
				++dev->scw_stats.pkts_65_127;
		if ((len >= 128) && (len <= 255))
				++dev->scw_stats.pkts_128_255;
		if ((len >= 256) && (len <= 511))
				++dev->scw_stats.pkts_256_511;
		if ((len >= 512) && (len <= 1023))
				++dev->scw_stats.pkts_512_1023;
		if ((len >= 1024) && (len <= 1518))
				++dev->scw_stats.pkts_1024_1518;
#endif
	skip:
...
#ifdef CONFIG_SIMATIC_NET_SCALANCE_W
static int scw_get_eth_stats(struct emac_instance *dev, void __user *usraddr)
{
	struct scw_eth_stats *sst = &dev->scw_stats;
	struct emac_stats *st = &dev->stats;
	struct emac_error_stats *est = &dev->estats;

	sst->drop_events = est->rx_dropped_oom + est->rx_dropped_error
						+est->rx_dropped_resize + est->rx_dropped_mtu;
	sst->rx_bytes = st->rx_bytes;
	sst->rx_all_pkts = st->rx_packets;

	/* bcast_pkts and multi_pkts are counted as soon as they are
	 * received in emac_poll_rx
	 */

	sst->crc_align_err = est->rx_bd_bad_fcs + est->rx_bad_fcs
						+ est->rx_bd_alignment_error
						+ est->rx_alignment_error;
	sst->undersize_pkts = est->rx_bd_short_event + est->rx_short_event;
	sst->oversize_pkts = est->rx_packet_too_long;

	/* fragments and jabber not available */

	sst->collisions = est->tx_bd_excessive_deferral +
            			est->tx_bd_excessive_collisions +
            			est->tx_bd_late_collision +
            			est->tx_bd_multple_collisions;

	/* pkt sizes are counted in the receive function nemac_poll_rx */

	return copy_to_user(usraddr, sst, sizeof(struct scw_eth_stats));
}

static int scw_reset_eth_stats(struct emac_instance *dev)
{
	struct scw_eth_stats *sst = &dev->scw_stats;
	struct emac_stats *st = &dev->stats;
	struct emac_error_stats *est = &dev->estats;

	memset(sst, 0, sizeof (struct scw_eth_stats));
	memset(st, 0, sizeof (struct emac_stats));
	memset(est, 0, sizeof (struct emac_error_stats));
	return 0;
}

static int scw_access_phy_reg(struct emac_instance *dev, void __user *usradd)
{
	struct mii_phy *phy = &dev->phy;
	struct scw_mii_phy mii_phy;

	if (copy_from_user(&mii_phy, usradd, sizeof(struct scw_mii_phy)))
		return -EFAULT;

	/* MSB is set if it is a write transaction */
	if (mii_phy.addr & (1<<31)){
		mii_phy.addr &= ~(1<<31);
		emac_mdio_write(phy->dev, phy->address, mii_phy.addr, mii_phy.value);
		return 0;
	}

	/* Read Transaction */
	mii_phy.value = emac_mdio_read(phy->dev, phy->address, mii_phy.addr);
	if (mii_phy.value < 0)
		return -1;
	return copy_to_user(usradd, &mii_phy, sizeof(struct scw_mii_phy));
}
...
static int emac_ioctl(struct net_device *ndev, struct ifreq *rq, int cmd)
{
	...
	case SIOCDEVPRIVATE + 13:
		return scw_access_phy_reg(dev, rq->ifr_data);
	case SIOCDEVPRIVATE + 14:
		return scw_reset_eth_stats(dev);
	case SIOCDEVPRIVATE + 15:
		return scw_get_eth_stats(dev, rq->ifr_data);
#endif
	default:
		return -EOPNOTSUPP;
	}
...
#ifdef CONFIG_SIMATIC_NET_SCALANCE_W
	/* If it's sfp based configure GMII instead */
	/* DD: EAP-SFP has hw_type 8. The board types should be defined someplace
	 * and we should probably have a struct with common data e.g. hwtype that
	 * is filled up once at startup.
	 */
	if (scw_get_hwtype() == 8) {
		dev->phy_mode = PHY_MODE_GMII;
		dev->phy_map = 0xf;
	}
#endif
