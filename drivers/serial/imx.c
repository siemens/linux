...
#else
#define  UCR1_UARTCLKEN  (1<<2)	 /* UART clock enabled, mx1 only */
#endif
...
struct uart_port *imx_dport=NULL;
void do_putchar_imx(char c)
{
	if(imx_dport != NULL)
		imx_console_putchar(imx_dport,c);
}
...
printk("bh3672: set imx_dport \n");
	imx_dport = &(sport->port);
