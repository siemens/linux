...
static void mpc8xxx_wdt_timer_ping(unsigned long arg)
{
#if CONFIG_AUD_DISABLE_WDT_AUTOMODE
	/* driver must not reset the watchdog automatically,
	 * after the device is closed; 
	 * this is only done by application firmware
	 */
#else
...
#if CONFIG_AUD_DISABLE_WDT_AUTOMODE
	/* driver must not activate the watchdog;
	 * this is done by uboot (when we boot from flash)
	 */
#else
