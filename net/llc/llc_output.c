...
{
	int rc;
	unsigned char lsap = sap->laddr.lsap;

#ifdef CONFIG_LLC_DOWNLOAD
	/* patch for firmware download; use SAPs 0xFC instead of 0xFD because the right bit of the SAPs have special meaning: 
	 * IG Bit in dsap: individual/group bit; CR Bit in ssap: command/response bit
	 */
	if (lsap == 0xFC)
	{
	    lsap = 0xFD;
	...
	llc_pdu_header_init(skb, LLC_PDU_TYPE_U, lsap,
			    dsap, LLC_PDU_CMD);
