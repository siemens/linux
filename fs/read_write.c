...
static struct type_info stdout = {1,LX_OUT_MSG, 5};
static struct type_info stderr = {2,LX_ERR_MSG, 5};
#endif
...
#if defined(CONFIG_RTX_DOMAIN) && !defined(CONFIG_RTX_DISABLE_WRITE_DAEMON)
	/* The write daemon handles printf to stdout/stderr for NRT-threads. */
	if ((current->rt_state == LXRT_TASK) && WRITE_DAEMON_IS_ACTIVE(current)) {
		switch (fd) {
			case 1:
			ret = rt_write_buffered(1, &stdout, (void *)buf, count);
			return ret;

			case 2:
			ret = rt_write_buffered(1, &stderr, (void *)buf, count);
			return ret;
		}
	}
#endif
