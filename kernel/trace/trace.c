...
#ifdef CONFIG_RTX_DOMAIN
DEFINE_PER_CPU(int, rt_domain_trace);
#endif
...
{
#ifdef CONFIG_RTX_DOMAIN
	// prevent scheduling when running in the RT domain
	// TBD: we may do a wakeup when running as LX thread of a RT process
	if (current->rt_state)
		return;
#endif
	/*
...
#ifdef CONFIG_RTX_DOMAIN
	if (rtx_get_cpu_var(rt_domain_trace)) {
		rtx_set_cpu_var(rt_domain_trace, 0);
		entry->ent.flags |= TRACE_FLAG_RT_DOMAIN_TRACE;
	}
#endif
