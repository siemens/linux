...
# define spin_lock_init(lock)						\
	do {								\
		if (TYPE_EQUAL((lock), __rtx_spinlock_t))		\
			do {						\
				RTX_DEFINE_SPINLOCK(__lock__);	\
				*((rtx_spinlock_t *)lock) = __lock__; \
			} while(0);					\
		else							\
...
do {									\
	if (TYPE_EQUAL((lock), __rtx_spinlock_t))			\
		__raw_spin##op(&((__rtx_spinlock_t *)(lock))->__raw_lock); \
...
do {									\
	if (TYPE_EQUAL((lock), __rtx_spinlock_t)) {			\
		__rtx_spin_lock_irq(&((__rtx_spinlock_t *)(lock))->__raw_lock); \
	} else if (TYPE_EQUAL(lock, spinlock_t))			\
...
do {									\
	if (TYPE_EQUAL((lock), __rtx_spinlock_t)) {			\
		__rtx_spin_unlock_irq(&((__rtx_spinlock_t *)(lock))->__raw_lock); \
	} else if (TYPE_EQUAL(lock, spinlock_t))			\
...
do {									\
	if (TYPE_EQUAL((lock), __rtx_spinlock_t)) {			\
		(flags) = __rtx_spin_lock_irqsave(&((__rtx_spinlock_t *)(lock))->__raw_lock); \
	} else if (TYPE_EQUAL(lock, spinlock_t))			\
...
#define PICK_SPINUNLOCK_IRQRESTORE(lock, flags)				\
	do {								\
	if (TYPE_EQUAL((lock), __rtx_spinlock_t)) {			\
		__rtx_spin_unlock_irqrestore(&((__rtx_spinlock_t *)(lock))->__raw_lock, flags); \
	} else if (TYPE_EQUAL(lock, spinlock_t))			\
...
# define spin_unlock(lock) \
    do {PICK_SPINOP_RAW(_unlock, lock); __release(lock); } while (0)
...
/* mqueue specific */
#define rtx_spin_lock_init(l) 	spin_lock_init(l)

#define rtx_spin_unlock_irqrestore(lock, flags) \
	spin_unlock_irqrestore(lock, flags)

#define rtx_spin_lock(lock) 	spin_lock(lock)
#define rtx_spin_unlock(lock) 	spin_unlock(lock)

#define rtx_read_seqbegin(l)	read_seqbegin(l)
#define rtx_read_seqretry(l, s)	read_seqretry(l, s)

#define rtx_spin_lock_irqsave(lock, flags) \
	spin_lock_irqsave(lock, flags)
#define rtx_spin_unlock_irqrestore(lock, flags) \
	spin_unlock_irqrestore(lock, flags)
...
#define spin_unlock_irqrestore_cond(lock, flags) \
	spin_unlock(lock)
#define __rtx_spin_lock_irq(lock)		do { } while(0)
#define __rtx_spin_unlock_irq(lock)		do { } while(0)
#define __rtx_spin_lock_irqsave(lock)		0
#define __rtx_spin_unlock_irqrestore(lock, x)	do { (void)(x); } while(0)
#endif /* RTX_DOMAIN */
